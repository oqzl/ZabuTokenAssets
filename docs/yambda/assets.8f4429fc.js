...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../node_modules/phina.js/build/phina.js":[function(require,module,exports) {
...
var _phina = _interopRequireDefault(require("phina.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var SCREEN_WIDTH = 1200;
var SCREEN_HEIGHT = 675;
var ENRAC_WIDTH = 278;
var ENRAC_HEIGHT = 380;
var ENRAC_INTERVAL = 42;
var PLAYER_SPEED = 8;
var PLAYER_WIDTH = 280;
var PLAYER_HEIGHT = 332;
var ENEMY_SPEED = 6;
var ENEMY_WIDTH = 160;
var ENEMY_HEIGHT = 120;
var ENEMY_INTERVAL = 150;
var BOTTOM_MERGIN = 120;
var MAX_ENEMY_COUNT = 7;
var GRAVITY = 0.3;
var SCORE_INTERVAL = 20;
var ASSETS = {
  image: {
    bg: "https://oqzl.github.io/ZabuTokenAssets/images/bg_main.png",
    enrac: "https://oqzl.github.io/ZabuTokenAssets/images/enrac.png",
    yambda: "https://oqzl.github.io/ZabuTokenAssets/images/oogiri_zabuton.png",
    enemy0: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_red.png",
    enemy1: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_black.png",
    enemy2: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_blue.png",
    enemy3: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_gray.png",
    enemy4: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_green.png",
    enemy5: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_orange.png",
    enemy6: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_yellow.png",
    enemy7: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_brown.png",
    enemy8: "https://oqzl.github.io/ZabuTokenAssets/images/zabuton_purple.png"
  },
  font: {
    DotGothic16: "https://oqzl.github.io/ZabuTokenAssets/fonts/DotGothic16-Regular.ttf"
  },
  spritesheet: {
    yambda_ss: {
      frame: {
...
_phina.default.globalize();

_phina.default.define("TitleScene", {
  ...
  init: function init() {
    ...
    var ymd = Player().addChildTo(this).setPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2).setScale(-1.5, 1.5);
    ymd.alpha = 0.4;
    Label({
      x: SCREEN_WIDTH / 2,
      y: SCREEN_HEIGHT / 3,
    ...
    Label({
      x: SCREEN_WIDTH / 2,
      y: SCREEN_HEIGHT / 4 * 3,
      text: "HIT SPACE OR TOUCH TO START",
      fontFamily: "DotGothic16",
      fontSize: 40
    }).addChildTo(this).tweener.fadeOut(500).fadeIn(500).wait(500).setLoop(true).play();
  },
  update: function update(app) {
    var p = app.pointer;
    var k = app.keyboard;

    if (p.getPointingStart() || k.getKeyDown("space")) {
      this.exit();
    }
  }
});

_phina.default.define("MainScene", {
  superClass: "DisplayScene",
  tick: 0,
  init: function init() {
    this.superInit({
    ...
    Sprite("bg", SCREEN_WIDTH, SCREEN_HEIGHT).addChildTo(this).setPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2);
    this.enrac = Enrac().addChildTo(this);
    this.player = Player().addChildTo(this);
    this.enemyGroup = DisplayElement().addChildTo(this);
    this.generateEnemy(); // レベル初期化
  ...
  update: function update(app) {
    ...
    if (p.getPointingStart() || k.getKeyDown("space")) {
      this.player.reflectX(); // this.player.physical.velocity.x *= 3;
    }

    if (p.getPointingEnd() || k.getKeyUp("space")) {// this.player.physical.velocity.x /= 3;
    }

    this.tick++;

    if (this.tick % SCORE_INTERVAL === 0) {
      score += 10;
      scoreLabel.text = "SCORE: " + score;
    }

    if (this.tick % ENEMY_INTERVAL === 0) {
      var enemys = this.enemyGroup.children;

      if (enemys.length < MAX_ENEMY_COUNT) {
        this.generateEnemy(enemys.length);
      } else {
        level++;
      }
    }
  ...
  hitTest: function hitTest() {
    var hit = false;
    var p = this.player;
    var cp = Circle(p.x, p.y, PLAYER_WIDTH / 3);
    this.enemyGroup.children.each(function (e) {
      var ce = Circle(e.x, e.y, ENEMY_HEIGHT / 2);

      if (Collision.testCircleCircle(cp, ce)) {
        hit = true;
        return false;
      }
    });
  ...
  generateEnemy: function generateEnemy() {
    var numEnemys = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var x = Random.randint(0, SCREEN_WIDTH);
    var y = 0; //ENEMY_HEIGHT;
    // var x = this.enrac.x;
    // var y = this.enrac.y - ENEMY_HEIGHT;

    var type = Random.randint(0, Math.min(7, numEnemys + 2));
    Enemy(type).addChildTo(this.enemyGroup).setPosition(x, y);
  }
});

_phina.default.define("ResultScene", {
  superClass: "DisplayScene",
  init: function init() {
    this.superInit({
      width: SCREEN_WIDTH,
    ...
    var enrac = Enrac(2.5).addChildTo(this).setPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT * 0.8);
    enrac.alpha = 0.4;
    Label({
      x: SCREEN_WIDTH / 2,
      y: SCREEN_HEIGHT / 2,
...
_phina.default.define("Enrac", {
  superClass: "Sprite",
  viewScale: 1,
  tick: 0,
  init: function init() {
    var viewScale = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
    this.superInit("enrac");
    this.setSize(ENRAC_WIDTH, ENRAC_HEIGHT);
    this.setPosition(SCREEN_WIDTH / 7, SCREEN_HEIGHT / 2);
    this.setScale(viewScale, viewScale);
  ...
  update: function update() {
    ...
    if (this.tick % ENRAC_INTERVAL === 0) {
      console.log("ENRAC Tween");
      this.tweener.scaleTo(this.viewScale * 1.02, 150).scaleTo(this.viewScale, 50).play();
    }
  }
});

_phina.default.define("Player", {
  superClass: "Sprite",
  init: function init() {
    this.superInit("yambda");
    this.setSize(PLAYER_WIDTH, PLAYER_HEIGHT);
    var anim = FrameAnimation("yambda_ss").attachTo(this);
    anim.gotoAndPlay("walk");
    this.setPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT - BOTTOM_MERGIN - PLAYER_HEIGHT / 2);
  ...
  reflectX: function reflectX() {
    this.physical.velocity.x *= -1;
    this.scaleX *= -1;
  }
});

_phina.default.define("Enemy", {
  superClass: "Sprite",
  rotate: 0,
  init: function init(type) {
    this.superInit("enemy" + type);
    this.setSize(ENEMY_WIDTH, ENEMY_HEIGHT);
    this.setPosition(SCREEN_WIDTH / 2, ENEMY_HEIGHT);
    this.physical.gravity.y = GRAVITY;
    this.physical.velocity.x = (type + 1) / 2 * ENEMY_SPEED; // this.physical.velocity.y = -ENEMY_SPEED * 1.5;
  },
  update: function update() {
    var ground_bottom = SCREEN_HEIGHT - BOTTOM_MERGIN;

    if (this.bottom > ground_bottom) {
      this.bottom = ground_bottom;
    ...
    if (this.left < 0) {
      this.left = 0;
      this.reflectX();
      this.rotate += Random.randint(0, 3);
  ...
  speedUp: function speedUp(level) {}
});

_phina.default.main(function () {
  var app = GameApp({
    startLabel: "title",
    scenes: SCENES,
    title: "YambdaGo",
...
},{"phina.js":"../node_modules/phina.js/build/phina.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
